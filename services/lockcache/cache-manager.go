	...
	for {
		...
		case None:
			...
			if *acqResp.Success {
				...
				cm.logger.Infof("[CacheManager] Lock %s acquired (owner=%s, seq=%d)", req.LockId, req.OwnerId, req.Sequence)
		...
		case Free:
			...
			cm.logger.Infof("[CacheManager] Reused cached lock %s (owner=%s)", req.LockId, req.OwnerId)
			return &lockapi.AcquireResponse{Success: proto.Bool(true)}, nil

		case Locked:
			cacheInfo.mu.Lock()
			cm.logger.Infof("[CacheManager] Waiting for lock %s...", req.LockId)
			cacheInfo.cond.Wait()
			cacheInfo.mu.Unlock()

		default:
			cm.logger.Warnf("[CacheManager] Unknown state %s for lock %s", state, req.LockId)
			return &lockapi.AcquireResponse{Success: proto.Bool(false)}, nil
		}
	}
}
...
func (cm *CacheManager) Release(ctx context.Context, req *lockapi.ReleaseRequest) (*lockapi.ReleaseResponse, error) {
	...
	if cacheInfo == nil {
		return nil, fmt.Errorf("release: lock %s not found in cache", req.LockId)
	}
	...
	default:
		return nil, fmt.Errorf("release: invalid state for lock %s (%s)", req.LockId, cacheInfo.State)
	}
}
